...
export interface DislikedItem {
  ...
  /** Latest proposed correct answer across all operators (single
   *  most-recent row, MAX(created_at) per answer_id). null when no
   *  operator has submitted a follow-up correction yet. */
  correction: {
    uid: string
    correction: string
    created_at: string
  } | null
  /** Number of distinct uids that submitted a correction for this
   *  answer. Zero means "no follow-up yet" (correction will be
   *  null). >1 means multiple operators independently reached the
   *  same conclusion. */
