...
/** First 8 chars of the operator uid — enough to distinguish
 *  contributors in a small ops team without leaking the full UUID. */
function shortUid(uid: string): string {
  return uid.length > 8 ? uid.slice(0, 8) : uid
}

/** Human "egy / több" phrasing for the correction-count badge. */
function correctionCountLabel(n: number): string {
  if (n === 0) return '—'
  if (n === 1) return '1 javaslat'
  return `${n} javaslat`
}
        ...
        >
          ...
          >
            <div class="flex items-baseline justify-between gap-3 min-w-0">
              <div class="min-w-0">
                <div class="text-[13.5px] font-medium text-[var(--nct-form-text)] truncate flex items-center gap-2">
                  <span class="truncate">{{ item.q }}</span>
                  <span
                    v-if="item.correction_count > 0"
                    class="shrink-0 inline-flex items-center gap-1 px-1.5 h-5 rounded-md
                           bg-emerald-500/10 text-emerald-700 dark:text-emerald-300
                           border border-emerald-500/25 text-[10.5px] font-medium
                           whitespace-nowrap"
                    :title="`${item.correction_count} operátor küldött javított választ`"
                    :data-testid="`disliked-answers-correction-badge-${item.answer_id}`"
                  >
                    ✓ {{ correctionCountLabel(item.correction_count) }}
                  </span>
                </div>
      ...
      >
        ...
        >
          ...
          <div class="px-5 py-4 space-y-4">
            ...
            <!-- Proposed correct answer (operator "Küldd el a fejlesztésnek!") -->
            <div
              v-if="selected.correction"
              data-testid="disliked-answers-drawer-correction"
            >
              <div class="flex items-baseline justify-between gap-2">
                <div class="text-[10.5px] font-mono uppercase tracking-wider text-emerald-700 dark:text-emerald-300">
                  Javasolt helyes válasz
                </div>
                <div class="text-[10.5px] font-mono text-[var(--nct-form-text-muted)] tabular-nums">
                  <span :data-testid="`disliked-answers-drawer-correction-uid-${selected.answer_id}`">
                    {{ shortUid(selected.correction.uid) }}
                  </span>
                  ·
                  <span :data-testid="`disliked-answers-drawer-correction-at-${selected.answer_id}`">
                    {{ fmtDate(selected.correction.created_at) }}
                  </span>
                  <span
                    v-if="selected.correction_count > 1"
                    class="ml-1 px-1.5 h-4 inline-flex items-center rounded
                           bg-emerald-500/10 text-emerald-700 dark:text-emerald-300
                           border border-emerald-500/25 text-[10px] font-medium"
                    :data-testid="`disliked-answers-drawer-correction-count-${selected.answer_id}`"
                    :title="`Összesen ${selected.correction_count} operátor küldött javaslatot erre a válaszra`"
                  >
                    {{ correctionCountLabel(selected.correction_count) }}
                  </span>
                </div>
              </div>
              <div
                class="mt-1.5 text-[13.5px] text-[var(--nct-form-text)] leading-relaxed
                       whitespace-pre-wrap break-words rounded-lg
                       border border-emerald-500/30 bg-emerald-500/[0.07] px-3 py-2.5"
                :data-testid="`disliked-answers-drawer-correction-text-${selected.answer_id}`"
              >
